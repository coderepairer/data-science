...
def answer_two():
    ...
    accuracy = accuracy_score(y_test,dummy_predicted)
...
def answer_three():
    ...
    accuracy = accuracy_score(y_test, svc_predicted)
...
def answer_four():
    ...
    svc_dec_fn = svc.decision_function(X_test) > -220
    #svc_dec_fn = svc_dec_fn[svc_dec_fn > -220]
    return confusion_matrix(y_test,svc_dec_fn)
...
def answer_five():
    ...
    y_lr_predict = lr.fit(X_train, y_train).predict_proba(X_test)[:,1]
    precision, recall, _ = precision_recall_curve(y_test, y_lr_predict)
    ...
    val1= np.interp(0.75, precision, recall)
